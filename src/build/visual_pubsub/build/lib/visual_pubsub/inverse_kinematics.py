...
class InverseKinematics(Node):
    ...
    def __init__(self):
        ...
        self.target_pos = np.array([4.0, 2.0, 1.5 ])  # Default target
    ...
    def forward_kinematics(self, q):
        q1, q2, d4 = q
        x = l2 * np.cos(q1) + l3 * np.cos(q1 + q2)
        y = l2 * np.sin(q1) + l3 * np.sin(q1 + q2)
        z = -d4 + l1
        return np.array([x, y, z])

    def jacobian(self, q):
        q1, q2, q3 = q
        j11 = -l2*np.sin(q1) - l3*np.sin(q1 + q2)
        j12 = -l3*np.sin(q1 + q2)
        J13 = 0.0

        j21 =  l2*np.cos(q1) + l3*np.cos(q1 + q2)
        j22 =  l3*np.cos(q1 + q2)
        j23 = 0.0
